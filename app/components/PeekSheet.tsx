...
export default function PeekSheet({ parkings, onSelect, freeCount, paidCount, timestamp }: Props) {
  ...
  const onTouchMove = useCallback((e: React.TouchEvent) => {
    ...
    if (state === "collapsed") { const base = sheetRef.current.offsetHeight - 140; sheetRef.current.style.transform = `translateY(${Math.max(-base, base + dy)}px)`; }
    else if (state === "expanded" && dy > 0) { sheetRef.current.style.transform = `translateY(${dy}px)`; }
  }, [state]);
  const onTouchEnd = useCallback(() => {
    if (!dragging.current) return; dragging.current = false; sheetRef.current?.classList.remove("sheet-dragging");
    const dy = currentY.current - startY.current;
    if (state === "expanded") setState(dy > 80 ? (dy > 200 ? "hidden" : "collapsed") : "expanded");
    else if (state === "collapsed") setState(dy < -60 ? "expanded" : dy > 60 ? "hidden" : "collapsed");
  ...
  return (
    <div ref={sheetRef} className="fixed left-0 right-0 z-[1000] bg-white dark:bg-[#131318] border-t border-black/8 dark:border-white/8 rounded-t-[20px] flex flex-col shadow-[0_-4px_20px_rgba(0,0,0,0.08)]"
      ...
      <div className="flex items-center justify-between px-5 pb-3" onClick={toggle}>
        <span className="text-lg font-bold tracking-tight text-gray-900 dark:text-white">A proximite</span>
      ...
      <div className="flex gap-2 px-5 pb-3.5">
        <div className="flex-1 p-2.5 bg-gray-100 dark:bg-gray-800/50 rounded-xl text-center">
          ...
          <div className="text-[10px] text-gray-400 uppercase tracking-wide mt-0.5">Gratuit</div>
        </div>
        <div className="flex-1 p-2.5 bg-gray-100 dark:bg-gray-800/50 rounded-xl text-center">
          <div className="font-mono text-xl font-bold" style={{ color: "var(--paid)" }}>{paidCount}</div>
          <div className="text-[10px] text-gray-400 uppercase tracking-wide mt-0.5">Payant</div>
        </div>
      </div>
            ...
            <button key={p.id} onClick={() => onSelect(p)} className="w-full flex items-center gap-3.5 p-3.5 bg-gray-50 dark:bg-gray-800/30 border border-black/5 dark:border-white/5 rounded-[14px] mb-2 text-left active:bg-gray-100 dark:active:bg-gray-700">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <span className="text-sm font-semibold truncate text-gray-900 dark:text-white">{p.name}</span>
                  {p.realtime && <span className="w-1.5 h-1.5 rounded-full bg-green-500 shrink-0" title="Temps reel" />}
                </div>
