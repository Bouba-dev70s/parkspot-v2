...
export default function DetailSheet({ parking, onClose, isFav, onToggleFav, userPos, onParkHere }: Props) {
  ...
  const p = parking;
  const unknown = p && !p.realtime;
  const pct = p && p.total > 0 ? p.avail / p.total : 0;
  const vc = pct > 0.3 ? "var(--free)" : pct > 0 ? "var(--paid)" : "var(--full)";
  const dist = useMemo(() => { if (!p || !userPos) return null; const d = distanceKm(userPos[0], userPos[1], p.lat, p.lng); return { text: d < 1 ? `${Math.round(d * 1000)}m` : `${d.toFixed(1)}km`, walk: `~${Math.round(d * 12)} min` }; }, [p, userPos]);
  ...
  const servicesList: { icon: string; label: string }[] = [];
  if (svc.couvert) servicesList.push({ icon: "🏢", label: "Couvert" });
  if (svc.pmr) servicesList.push({ icon: "♿", label: "PMR" });
  if (svc.electrique) servicesList.push({ icon: "⚡", label: "Électrique" });
  if (svc.surveillance) servicesList.push({ icon: "👁", label: "Surveillé" });
  if (svc.velo) servicesList.push({ icon: "🚲", label: "Vélos" });
  ...
  return (
    <div ref={ref} className={`fixed bottom-0 left-0 right-0 z-[1800] bg-white dark:bg-[#131318] rounded-t-3xl border-t border-black/8 dark:border-white/8 shadow-[0_-8px_30px_rgba(0,0,0,0.1)] safe-bottom overflow-y-auto ${parking ? "sheet-visible" : "sheet-enter"}`}
      ...
      <div className="px-6 pb-5">
        ...
        <p className="text-xs text-gray-400 mb-4">{p.hours}{dist ? ` · ${dist.text} ${dist.walk}` : ""}{lastUpdateText ? ` · Maj ${lastUpdateText}` : ""}</p>
        <div className="flex gap-1.5 flex-wrap mb-4">{servicesList.map((s, i) => (<span key={i} className="flex items-center gap-1 px-2.5 py-1 bg-gray-100 dark:bg-gray-800/50 rounded-full text-[11px] font-medium text-gray-600 dark:text-gray-400">{s.icon} {s.label}</span>))}</div>
        <div className="grid grid-cols-3 gap-2.5 mb-4">
          <div className="bg-gray-100 dark:bg-gray-800/50 rounded-[14px] p-4 text-center"><div className="font-mono text-[28px] font-semibold" style={{ color: vc }}>{unknown ? "~" : ""}{p.avail}</div><div className="text-[11px] text-gray-400 mt-1">{unknown ? "Estimé" : "Disponibles"}</div></div>
        ...
        <div className="mb-4">
          <div className="flex justify-between text-xs text-gray-500 mb-2"><span>Occupation {unknown ? "(estimée)" : ""}</span><span style={{ color: vc }}>{unknown ? "~" : ""}{Math.round(pct * 100)}% disponible</span></div>
        ...
        {unknown && (<div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/10 rounded-xl"><div className="text-xs text-blue-500 font-medium">~ Estimation basée sur l&apos;heure et le jour. Données temps réel non disponibles.</div></div>)}
        <div className="mb-4"><div className="text-xs font-semibold text-gray-400 tracking-wide mb-2">Affluence estimée</div><div className="flex gap-[2px] items-end h-10">{hours.map((h, i) => (<div key={i} className="hour-bar flex-1" style={{ height: `${Math.max(4, h * 40)}px`, background: i === nowH ? "var(--accent)" : h > 0.7 ? "var(--full)" : h > 0.4 ? "var(--paid)" : "var(--free)", opacity: i === nowH ? 1 : 0.5 }} />))}</div></div>
        <div className="bg-gray-100 dark:bg-gray-800/50 rounded-xl p-4 mb-4">
          <div className="flex justify-between items-center mb-3"><span className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Estimation du prix</span><span className="font-mono text-lg font-bold" style={{ color: p.type === "free" ? "var(--free)" : "var(--paid)" }}>{est}</span></div>
