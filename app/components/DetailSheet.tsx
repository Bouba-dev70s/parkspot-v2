...
export default function DetailSheet({ parking, onClose, isFav, onToggleFav, userPos }: Props) {
  ...
  const distance = useMemo(() => {
    ...
    return d < 1 ? `${Math.round(d * 1000)}m · ~${Math.round(d * 12)} min` : `${d.toFixed(1)}km · ~${Math.round(d * 12)} min`;
  }, [p, userPos]);
  ...
  return (
    <div ref={ref} className={`fixed bottom-0 left-0 right-0 z-[1800] bg-white dark:bg-[#131318] rounded-t-3xl border-t border-black/8 dark:border-white/8 shadow-[0_-8px_30px_rgba(0,0,0,0.1)] safe-bottom overflow-y-auto ${parking ? "sheet-visible" : "sheet-enter"}`}
      ...
      <div className="px-6 pb-5">
        <div className={`text-[11px] font-semibold tracking-[2px] uppercase mb-1.5 ${p.type === "free" ? "text-[var(--free)]" : "text-[var(--paid)]"}`}>{p.type === "free" ? "Parking gratuit" : "Parking payant"}</div>
        <h2 className="text-2xl font-bold tracking-tight mb-1 text-gray-900 dark:text-white">{p.name}</h2>
        <p className="text-sm text-gray-400 dark:text-gray-500 mb-4">{p.addr} · {p.hours}{distance ? ` · ${distance}` : ""}{p.city ? ` · ${p.city}` : ""}</p>

        <div className="flex gap-2 flex-wrap mb-4">
          {["Couvert", "24/7", "PMR", "Electrique"].map((s) => (
            <span key={s} className="flex items-center gap-1 px-3 py-1 bg-gray-100 dark:bg-gray-800/50 rounded-full text-[11px] font-medium text-gray-500 dark:text-gray-400"><HiCheck size={12} /> {s}</span>
          ))}
        </div>
        ...
        <div className="bg-gray-100 dark:bg-gray-800/50 rounded-xl p-4 mb-4">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-gray-400 shrink-0">30min</span>
            <input type="range" min="0.5" max="24" step="0.5" value={duration}
              onChange={(e) => setDuration(parseFloat(e.target.value))}
              className="flex-1 accent-[var(--accent)]" style={{ height: "4px" }} />
