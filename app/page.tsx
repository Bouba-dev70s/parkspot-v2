        ...
        <div className="absolute inset-0">
          ...
          <div className="absolute top-3 left-4 right-4 z-[1000] safe-top">
            <div className="relative mb-3">
              <button onClick={changeCity} className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-[var(--accent)]/10 text-[var(--accent)] text-[11px] font-semibold active:opacity-70 mb-2"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>{(city?.name || "Ville").length > 18 ? (city?.name || "").substring(0, 18) + "…" : city?.name} ▾</button>
              <div className="relative">
                <svg className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8" /><path d="m21 21-4.3-4.3" /></svg>
                <input type="text" value={addrQuery} onChange={(e)=>onAddrChange(e.target.value)} placeholder="Rechercher une adresse..." className="w-full pl-11 pr-4 py-3 bg-white dark:bg-[#1c1c24] border border-black/8 dark:border-white/8 rounded-2xl text-[15px] outline-none shadow-[0_4px_20px_rgba(0,0,0,0.08)] text-gray-900 dark:text-white placeholder:text-gray-400" />
              </div>
